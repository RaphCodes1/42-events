...
export default function AdminPage() {
  ...
  const handleDelete = (eventId: string) => {
    alert(`Mock delete event with id: ${eventId}`);
  ...
  const handleSaveEvent = (event: Event) => {
    alert('Mock save event: ' + JSON.stringify(event));
          ...
          >
            {displayedEvents.map((event, index) => (
              <motion.div
                ...
                whileHover={{ y: -5 }}
              >
                <div className="aspect-video relative overflow-hidden cursor-pointer" onClick={() => handleEventClick(event)}>
                  <img
                    src={event.imageUrl}
                    alt={event.title}
                ...
                <div className="p-4">
                  <div className="flex items-center text-xs text-gray-600 mb-2">
                    ...
                    <span className="px-2 py-1.2 whitespace-nowrap" style={{ fontSize: '12px' }}>{event.date}</span>
